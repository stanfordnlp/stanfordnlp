...
def get_wordvec_file(wordvec_dir, shorthand, wordvec_type=None):
    ...
    if wordvec_type is not None:
        lang_dir = os.path.join(wordvec_dir, wordvec_type)
        if not os.path.exists(lang_dir):
            raise Exception("Word vector type {} was specified, but directory {} does not exist".format(wordvec_type, lang_dir))
        ...
        raise Exception("Cannot locate word vector directory for language: {}  Looked in {} and {}".format(lang, word2vec_dir, fasttext_dir))
